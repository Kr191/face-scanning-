...
def main():
    ...
    while True:
        ...
        if matchFound: 
            ...
            login_today = collection_login.find_one({"user_id": user_data["_id"], "loggedInAt": {"$regex": f"^{timestamp[:10]}"}})
          
            url = f"https://qyrqyyqbsqmexauttmpd.supabase.co/storage/v1/object/public/facescanningwithwebcam/images/{imgName[matchIndex]}"
            ...
            if login_today is None:
                ...
                if freeze_timestamp is None:
                    ...
                    collection_login.insert_one({"user_id": user_data["_id"], 
                ...
                cv2.putText(img, user_data['first_name'], (835, 360), cv2.FONT_ITALIC, 1, (0, 0, 0), 2)
                cv2.putText(img, user_data['last_name'], (835, 480), cv2.FONT_ITALIC, 1, (0, 0, 0), 2)
                cv2.putText(img, freeze_timestamp, (835, 600), cv2.FONT_ITALIC, 1, (0, 0, 0), 2)

            else:
                current_time = time.time() - last_logged_in_time
                if (user_data["_id"] == last_logged_in_id) and (current_time <= login_display_duration):
